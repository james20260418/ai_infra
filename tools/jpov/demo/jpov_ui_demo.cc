...
// 本 demo 显式注册两个字体：CJK（NotoSansCJK，显示中文）+ Latin（DejaVu，拉丁）。
// JPOV 不提供隐式默认字体，必须在 Config::fonts 显式声明 path/alias。
// Ui 文本指令的默认字体放在 UiTheme::font_alias（=CJK），见 OneIteration。
...
public:
    ...
    void OneIteration(int64_t frame_count,
        ...
        // 主题（16px flat 深色）。Ui 文本默认字体 = CJK（显中文）。
        jpov::UiTheme theme = jpov::UiTheme::Default(jpov::kUiDemoFontSize);
        theme.font_alias = kDemoFontAlias;
        ...
        ui_.End();
        // Emit 时 Ui 自动用主题默认字体（theme.font_alias）填充未指定别名的文本指令。
        ui_.Emit(cmds);
    }
