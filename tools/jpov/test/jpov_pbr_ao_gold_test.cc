...
//   2. 注册 baseColor 纹理 + 自发光贴图（4 个凸起半球，pbr_ao_256x256.png）
...
class PbrAoGoldTestApp : public JPOV {
public:
    using JPOV::JPOV;
...
int main() {
    ...
    JPOV::Config cfg;
    cfg.title = "PBR AO Gold Test";
    cfg.headless = true;
    PbrAoGoldTestApp app(cfg);
    app.Init();
    ...
    uint32_t ao_var = app.RegisterTexture(GetTexPath("pbr_ao_256x256.png"));
    ...
    LOG(INFO) << "TEST PASSED: PBR AO map (TBN) render pipeline ran "
