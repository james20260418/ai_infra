// JPOV CJK Text Gold Image Unit Test
//
// 验证 CJK 文本渲染正确：
//   1. 渲染字符串 "你好 世界！Hello World!"（字号 36，居中）
//   2. 保存为 PNG 到 output/jpov_text_cjk_gold_test/rendered.png
...
class CjkTextGoldTestApp : public JPOV {
public:
    using JPOV::JPOV;
...
int main() {
    ...
    std::string outdir = jpov::GetOutputDir() + "jpov_text_cjk_gold_test/";
    ...
    if (pass) {
        LOG(INFO) << "TEST PASSED: CJK text gold image match ("
                  << expected_bytes.size() << " bytes)";
        return 0;
    } else {
        LOG(ERROR) << "TEST FAILED: CJK text gold image mismatch";
        return 1;
    }
}
