...
std::string GetStampPath() {
    ...
    if (test_srcdir) {
        ...
        p += "__main__/tools/jpov/test/primitives2d/stamp_200x200.png";
        return p;
    }
...
std::string GetExpectedPngPath() {
    ...
    if (test_srcdir) {
        std::string p = test_srcdir;
        if (!p.empty() && p.back() != '/') p.push_back('/');
        p += "__main__/tools/jpov/test/primitives2d/image2d_stamp_640x360.png";
        return p;
    }
    return jpov::GetProjectRoot() +
           "tools/jpov/test/primitives2d/image2d_stamp_640x360.png";
}
