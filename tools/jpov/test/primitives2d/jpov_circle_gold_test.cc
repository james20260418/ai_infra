// JPOV Circle Gold Image Unit Test
//
// 用 gold image 方法验证 circle 渲染坐标正确：
//   1. 渲染一个已知位置的绿色圆形（居中，半径 100）
//   2. 保存为 PNG 到 output/jpov_circle_gold_test/rendered.png
...
class CircleGoldTestApp : public JPOV {
public:
    using JPOV::JPOV;
...
static std::string GetExpectedPngPath() {
    ...
    if (test_srcdir) {
        ...
        p += "__main__/tools/jpov/test/primitives2d/circle_centered_green_640x360.png";
        return p;
    }
    return jpov::GetProjectRoot() +
           "tools/jpov/test/primitives2d/circle_centered_green_640x360.png";
}
...
int main() {
    ...
    std::string outdir = jpov::GetOutputDir() + "jpov_circle_gold_test/";
    std::string outpath = outdir + "rendered.png";

    JPOV::Config cfg;
    cfg.title = "Circle Gold Test";
    cfg.headless = true;
    CircleGoldTestApp app(cfg);
    app.Init();
    ...
    if (pass) {
        LOG(INFO) << "TEST PASSED: circle gold image match ("
                  << expected_bytes.size() << " bytes)";
        return 0;
    } else {
        LOG(ERROR) << "TEST FAILED: circle gold image mismatch";
        return 1;
    }
}
