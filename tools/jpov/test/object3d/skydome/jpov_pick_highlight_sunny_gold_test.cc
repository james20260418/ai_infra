...
int main() {
    ...
    // ---- 高亮渲染 + 本体色断言 ----
    {
        ...
        // 新语义（总包络 + 透过遮挡）：高亮 wall 的边框会透过桌子的遮挡
        // 直接叠加在桌面上——这里验证“透过遮挡”确实发生（该区域应有金框），
        // 而非旧语义的“被前景挡住就不描”。
        // 区域 x[345,405] y[210,275]：wall 边框透过桌子右缘所在屏幕区域。
        {
            int table_gold = 0;
            ...
            LOG(INFO) << "木桌右缘(345-405,210-275) 透过遮挡金框像素 = " << table_gold
                      << "（新语义：墙边框透过遮挡应 > 0）";
            CHECK_GT(table_gold, 0)
                << "高亮边框未透过遮挡：验证 DrawHighlightPass 第2步是否已"
                   "关闭深度测试（被前景遮挡的高亮物体也应写剪影）。";
        }
