// JPOV PBR 多通道纹理 Gold Image Unit Test
//
// 用 gold image 方法验证 DrawObject3D 的 metallic/roughness/emissive/AO
// 纹理采样链路（逐个 sampler + 开关，采样进 BRDF，保留 kUV 校验）：
//   1. 从 OBJ 加载带 UV+normal 的立方体模型（cube_hand.obj）
//   2. 注册 5 个通道纹理：baseColor / metallic / roughness / emissive / AO
//   3. PBRMaterial 各 *_tex 指向纹理，走 GGX PBR 光照 + 逐像素材质采样
...
class PbrTexturesGoldTestApp : public JPOV {
public:
    using JPOV::JPOV;
...
int main() {
    ...
    std::string outdir = jpov::GetOutputDir() + "jpov_pbr_textures_gold_test/";
    std::string outpath = outdir + "rendered.png";

    JPOV::Config cfg;
    cfg.title = "PBR Textures Gold Test";
    cfg.headless = true;
    PbrTexturesGoldTestApp app(cfg);
    app.Init();
    ...
    LOG(INFO) << "TEST PASSED: PBR multi-channel texture render pipeline ran "
