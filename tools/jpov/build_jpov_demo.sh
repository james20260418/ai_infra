...
# jpov_demo 本身不渲染文本，但保持与其它 demo 一致的分发目录形态：
# 若未来加 DrawText，字体放在 exe 旁 tools/jpov/fonts/（与声明路径同构），
# ResolveFontPath 按 exe 相对路径命中。
echo "==> 4. 拷贝字体资源到 output/jpov_demo/tools/jpov/fonts/（jpov_demo 当前不画文本）"
...
echo ""
echo "   2. Windows 版 → 拷贝整个目录到 Windows 本地："
echo "       cp -r $OUTPUT_DIR /mnt/c/Users/<你的用户名>/Desktop/jpov_demo_gold"
echo "       然后在 Windows 资源管理器中进入该目录，双击 jpov_demo.exe 执行"
echo "       （exe 通过 GetModuleFileName 定位自身旁的相对路径字体，目录整体不能拆分）"
echo ""
echo "   提示：Windows 版 exe 为静态链接（无 .dll 依赖），目录需整体携带"
