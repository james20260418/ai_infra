...
- **自动推导 `SkyCommand::DirectionalIntensity()`（2026-08-22 改，2026-08-31 调参）**：
  ...
  经验仰角表（太阳仰角° → 相对正午照度系数，2026-08-31 当前实测标定版）：
    `1°→0  2°→0.23  5°→0.77  10°→0.77  30°→0.91  45°→1.0  90°→1.0`
  仰角 <0° / >90° 时 PWL 夹断到端点系数，不随越界外推。
  混度乘子 `TurbSunLoss(turb)` 见【第九节】；turb=2（大晴）时为 1.0，不改变晴空基准。
  - **注（2026-08-31 调参记录）**：`midday_intensity` 默认由 3.0 改为 2.2，且仰角表整体
    下调（旧版 0°→0.01 … 90°→1.0 改为 {1,2,5,10,30,45,90}° 的经验点）——这是 Danis
    gltf viewer 实测后肉眼标定的晴空基准，不一定贴合真实 DNI 量级，但 ACES 下观感协调。
...
- `color`：环境光**色调**（天空日间可偏蓝，洞穴岩浆可偏橙，夜空偏深蓝）。
  - 混度影响色调：`AmbientColor()` 里 turb 越大，haze 系数 `clamp((turb−2)/6,0,1)×0.6`
...
> **📌 当前实际取值提醒（2026-08-31）**：上表 3.0/0.3 是文档定稿的“基准值”；gltf viewer
> 的强度现在全由 sky 自动推导（强度滑条已移除），`DirectionalIntensity` midday 默认
> **2.2**、`AmbientIntensity` noon 默认 1.0 且正午再乘天光衰减（见 view_config.h/b 节）。
> 5:1 的比例关系依然成立（2.2 : 0.44≈5:1 时 shadow 表现等价），只是绝对基座从 3.0 挪到
> 了 2.2。后续若统一回 3.0，只需改 `DirectionalIntensity()` 的 midday 默认值，比例不变。
...
| 函数 | 作用对象 | 物理依据 | 衰减趋势 |
|---|---|---|---|
| `TurbSunLoss(turb)` | 太阳直射 `DirectionalIntensity()` | DNI 单一路径穿大气，近似指数吸收 exp(−τ·AM) | **陡**：晴→阴掉 ~5 倍 |
| `TurbAmbLoss(turb)` | 天光漫射 `AmbientIntensity()` | DHI 多路径散射，非单一路径 | **平**：薄云略升，重霾才明显降 |

> **为什么两条曲线不同**：直射（DNI）随浊度强衰减（尤其低仰角）；而散射（DHI）在
> 薄到中云时反而略升（更多直射被散射成漫射），到重霾才明显下降。因此这两个乘子
> **不能共用**，否则阴天会把物体外光人为做得过量暗。

### 2. 锚点曲线（PWL，以 turb=2 大晴 = 1.0 为基准）

```
TurbSunLoss(turb)： 2→1.00  3→0.80  4→0.60  5→0.40  6→0.30  8→0.20
TurbAmbLoss(turb)： 2→1.00  3→1.08  4→1.05  5→0.90  6→0.70  8→0.50
```

- turb<2 夹断到 1.0（更通透的天不增亮）；turb>8 夹断到端点值。
- **`TurbSunLoss(8)=0.20` 是“ACES 观感折中”后的 Danis 实测值**：物理上真阴天（整片云）
  直射可更低，但 JPOV 走 ACES（基准落在线性区），压太低会被非线性曲线压成死黑。
  初版 0.02 实测过暗（aces 贴地），调 0.06 后 Danis 在 gltf viewer 继续调至 **0.20**
  （阴天仍保留明显直射剪影），turb=6→0.30。见下文“物理 vs 观感”。

### 3. 浊度分级（Linke turbidity 物理标尺，供设滑条/理解用）

| Linke T | 物理含义 | 直射相对清透天 |
|---|---|---|
| 1.5~2 | 极清透（高山/雨后）| ~1.0 |
| **2** | **清透晴天（基准）** | 1.0 |
| 3~4 | 正常晴天（城市/郊区）| ~0.7~0.6 |
| 5 | 轻度霾 / 工业轻微污染 | ~0.5 |
| **6** | **中度霾 / 城市污染** | ~0.35 |
| 7~8 | 较重霾 / 沙尘 | ~0.2~0.1 |
| >10 | 严重污染 / 沙尘暴 | <0.05 |

> **⚠️ 语义提醒**：Linke turbidity 描述的是**气溶胶/霾度**，**不直接等于云量/阴雨**。
> 真阴天（整片云）的直射骤降主因是云层反射遮挡，等效浊度需 10+ 才像。JPOV 的 turb 滑条
> 目前在 gltf viewer 里被当作**“总体阴沉度”旋钮**（2=晴 → 8=重雾/最阴），这是观感用法，
> 物理上 turb=8 更准确的标签是“重霾/沙尘”，不是“阴雨天”。若需严格区分霾与云，需另加
> 云层模型（本设计暂不做）。

### 4. 物理衰减 vs ACES 观感折中（2026-08-31 调参关键结论）

- **纯 Beer-Lambert 物理**：turb=4/6/8 的 DNI 理论衰减约 0.5/0.27/0.17（τ≈0.7/1.3/1.8）。
- **但 JPOV 必须走 ACES 非线性压缩**，基准乘很小的系数后会被压到贴地、观感死黑。
- **结论**：不能只看物理衰减值，要按“`aces(intensity×Loss)` 是否落入可辨视区”定锚点。
  初版 `TurbSunLoss(8)=0.02` 实测过暗（aces 贴地），经 0.06 → Danis 定为 **0.20**。

### 5. 已知局限（2026-08-31 决策）

- **无空间“雾层”**：当前 turb 只衰减光照强度 + 调 sky/ambient 色调（发白），**没有**按距离
...
- **夜色项按软 (1−daylight) 淡入**（2026-09-21 方案 B）：白天项精确 0，但**夜色项
  必须在白天也门控**，否则会给正午蓝天抬底（实测 ~5/255，物理上错）。两者用
  **同一个 daylight 因子**，保证天色 / 环境光色 / 环境光强是同一条时间轴。
