...
echo "   窗口底部居中 9 个半屏宽滑条："
echo "     ① 太阳仰角 ° [0,90]（0=日落 → 白天项精确归零，画面只剩夜色；90=正午）"
echo "     ② 浊度 turb [2,8]（同时影响月晕宽度）"
...
echo "     ⑦ 月亮仰角 ° [0,90]（与太阳仰角独立；0=整盘淹没在地平线下）"
echo "     ⑧ 月盘亮度 [0,500]（0=不画月盘；默认 100 已可见）"
echo "     ⑨ 月晕强度 [0,10]（0=无月晕；配合②看晕宽随浊度变化）"
echo ""
echo "   验收要点（夜色）：把①滑到 0° → 看到「经典城市夜色」（天顶深、地平暖）；"
echo "   再把⑥拉到 0 → 对照「没有夜色的同一构图」。"
echo "   验收要点（月盘）：⑧ 拉到 100 左右 → 月盘出现在 −X 侧（画面左侧）；"
echo "   ⑦ 滑到 0° → 整盘淹没；⑨ 拉到 3 后拉② 2→8 → 月晕明显变宽。"
