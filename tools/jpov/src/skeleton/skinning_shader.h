...
// 因此 FS 端直接复用 kMeshFs3dPBRFull（同光照），保证 M1 sunny-day 效果与 object3d 一致。
...
void main() {
    ...
    // monté cloth 后的模型变换（center/up/front/scale）：uModel。
    vec4 world = uModel * vec4(sp, 1.0);
    vWorldPos = world.xyz;
    // 法线/切线沿用 object3d 的逆置约定（mat3(transpose(inverse(uModel)))）——
    // 绑定时 M_i=I，sn=aNormal，与 kMeshVs3dPBRFull 完全一致 ⇒ M1 diff≈0。
...
// {kSkinnedVs, kMeshFs3dPBRFull} 注册 program（见 object3d_renderer.h 导出）。
