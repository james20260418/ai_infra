// JPOV Renderer 实现
//
// FBO 固定 4096×4096，所有 2D 坐标在 FBO 空间中。
// Present 时窗口大小区域 → framebuffer（无缩放）。
...
unsigned int CompileShader(GLenum type, const char* source) {
    ...
    if (!ok) {
        ...
        const char* type_name = (type == GL_VERTEX_SHADER) ? "VS" : "FS";
...
// ============================================================
// RenderCommandList 方法定义（声明在 render_command.h 中）
// ============================================================
...
void Renderer::CreateFBO() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kFboWidth, kFboHeight, 0,
    ...
    LOG(INFO) << "Renderer: FBO ready (" << kFboWidth << "x" << kFboHeight << ")";
}

void Renderer::CompileShaders() {
    prog_ = BuildProgram();
    CHECK_NE(prog_, 0u) << "Shader compilation failed";
    LOG(INFO) << "Renderer: shader compiled (prog=" << prog_ << ")";
}
...
void Renderer::Present(GLFWwindow* window, int window_width, int window_height) {
    ...
    // 从 FBO 左上角取窗口大小区域，blit 到默认 framebuffer（无缩放）
    int src_w = std::min(window_width, kFboWidth);
    int src_h = std::min(window_height, kFboHeight);
...
void Renderer::DrawPolyline2D(const Polyline2DCommand& cmd,
    ...
    raw_pts.reserve(cmd.vertices.size() * 2);
    ...
    size_t upload_bytes = raw_pts.size() * sizeof(float);
    glBindBuffer(GL_ARRAY_BUFFER, stream_vbo_);
    glBufferData(GL_ARRAY_BUFFER, upload_bytes, raw_pts.data(), GL_DYNAMIC_DRAW);

    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)0);
    glDrawArrays(GL_LINE_STRIP, 0, static_cast<GLsizei>(raw_pts.size() / 2));
    glDisableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
