...
- `clip_` ← `LoadFbxAnimation`：`fps` + 全帧；帧里存的是**相对 bind 的增量旋转**（无量纲）
  + **相对 bind 的根位移 `root_offset`（长度量，源单位）**。

**旋转**是纯角度量、与单位无关 ⇒ “米制骨架 + 增量旋转”叠加无碍。两个入口的骨序同源（同一条
...
上面「旋转与单位无关」**不适用于 `root_offset`**（它是长度量）。本观察器下游几何**全是米制**
（`skeleton_` 米 / glb 米 / 地面米），而 `clip_` 帧的 `root_offset` 是**源单位**（Mixamo = cm）。

⇒ 凡把 `clip_` 的帧位姿交给米制下游（重定向、火柴人）前，**必须先**
`FbxViewerApp::NormalizePoseLengthsToMeters()`（= `root_offset × clip_.unit_meters`）。

不够一的后果（已实际发生）：重定向时用骨盆高做尺度比，把 cm 的位移配到 m 的骨架上 ⇒ 比值
静默错 **100 倍**（实测 |root_offset| 最大 **63.6 m**）⇒ 蓝骨人 / 蓝带皮**飞出画面**。
且当时的 gold 只盖了 naive 对照路径（`root_offset` 置 0）→ 静默通过；现已补上重定向路径的
数值 + 可见性 + gold 三道门禁（见 test/fbx_viewer/jpov_fbx_pose_gold_test.cc ⑤）。

关联测试：`test/fbx_viewer/jpov_fbx_retarget_units_test.cc`（含负向验证：忘记归一 ⇒ 超量级）。
...
### 3.6 「根位置矢量杆收窄 1/3」的判定（2026-09-14 修 bug）

火柴人里**沿根链的第一根可画杆** = "根位置矢量"（如 Mixamo 的 Hips：从骨架原点到骨盆，
近米级长，不是真骨骼段），半宽收窄为 `radius/3` 以便与真骨骼杆一眼区分。修的是判据：

- 旧判据 `parent == kSkeletonNoParent` 要求**根关节自己带长度** → 对"顶层是**零长包装
  Root**"的资产（本仓库 `Mixamo23Skeleton`、glb 资产）**静默失效**：变细的是那根看不见的
  包装杆，用户看到的 Hips 杆仍是普通粗细（Danis 报的"蓝骨 root 没变细"）。
- `mixamo_male.glb` 更刁：它的 Root **不是零长，而是 4.5mm 的导出残差** → 必须把"退化杆"
  定义清楚，否则残差会"抢走"根杆身份。
- 现规则（`interface/skeleton_mesh.h`）：**长度 < `kDegenerateRodLength`(1cm) 的杆视为退化
  包装残差、不画**；沿父链向上到根**没有任何可画杆**的那根 = 根位置矢量杆，收窄 1/3。
  1cm 的依据（实测）：人形资产最小**真骨** 21mm（fbx 手指）> 10mm > 4.5mm（glb Root 残差），
  两侧余量 >2×（假设米制人形尺度）。

回归锁定：`skeleton_mesh_test` 新增 3 例（零长包装 Root / 4.5mm 残差 Root / **三套真实资产
按骨名锁 Hips 杆收窄**）；受影响的两张 gold（`bone_mesh_stickman`、`skeleton_from_glb_fbx`）
已重生成（杆变细是有意变更）。**负向验证**：阈值退回 1e-8、或判据退回 `parent==NoParent`
→ 相关用例均 FAIL。
