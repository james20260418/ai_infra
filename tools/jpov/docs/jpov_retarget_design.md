...
> 本节是 **2026-09-15 与 Danis 逐轮推导后补写**的 —— 原文只给结论（"目标对齐⁻¹ · 源对齐 · 源旋转"）
> 而无推导，是后来实现走岔路的土壤。此处补全过程。
> 实现见 `interface/skeleton_retarget.h`（`BuildBind` 建 `Q` 表 + `RetargetPose` 逐帧 apply）。

#### 前提：关节 = 一个坐标系

一个关节的 local 系，原点在关节处、坐标轴朝向由旋转决定。所以"骨朝哪长" ⇔ "这个坐标系怎么摆"。
同一根**语义骨**（如左大腿）在两套 rig 上，局部系的摆放方式**通常不同**：
> 源骨架的局部 +Z 指大腿**前侧**；目标骨架的局部 +Z 指大腿**侧面**（Danis 的实测观察）。

"坐标系怎么摆"有两种描述：

| 描述 | 符号 | 含义 |
|---|---|---|
| **世界朝向** | `Rb(j)` / `W(j)` | 在骨架空间里，它的轴指向哪里 |
| **局部朝向** | `B(j)` | 相对它**父**的坐标系转了多少 |

两者沿树连乘（注意**左乘的是父**）：

```
W(j) = W(parent) ⊗ B(j) ⊗ P(j)          ← P = pose 的局部旋转
Rb(j) = Rb(parent) ⊗ B(j)               ← rest（P 全恒等）时的世界朝向
```

#### 桥接量 Q 的定义

两套骨架的 `Rb_S(j)` 与 `Rb_T(j)` **通常完全不同**（这就是"局部系摆放不同"）。
问：源关节系里的一个方向 `v_S`，搬到目标关节系里该写成什么？

```
v_T = Q(j) · v_S，其中   Q(j) ≜ Rb_T(j) ⊗ Rb_S(j)⁻¹
```

**读法**：Q 把"源的 rest 世界朝向"转到"目标的 rest 世界朝向"。

#### 为什么它可以只用 bind 朝向算一次（与帧无关）

Q 的两个输入 `Rb_T` / `Rb_S` 都只由 **`bind_rotation`**（骨架定义的一部分）决定，
**与动画帧无关** ⇒ **算一次，逐帧复用**。

> **这也回答了"FBX 是不是要逐帧求解重定向关系"**：不需要。逐帧只做一次固定乘法。
> 所谓"FBX 逐帧求解"说的是**源姿态本身的插值取值**（loader 已做完），不是重定向。

#### 逐帧 apply

源第 k 帧给每骨一个 `P_S(j)`（相对源 bind 的增量旋转，表达在源父关节系里）。
目标要的是"同一个物理旋转，表达在目标父关节系里"。这是**相似变换（共轭）**：

```
P_T(j) = Q(parent_T(j)) ⊗ P_S(j) ⊗ Q(j)⁻¹
         └─ 换输出系（父系）─┘   └─ 换输入系（本系）─┘
```

**理解要点**：一个旋转有"输入系"与"输出系"两端 —— 左乘 `Q(parent)` 换父系的解释，
右乘 `Q(j)⁻¹` 换本系的解释。**坐标变换下，同一变换的矩阵表示必须共轭过去** —— 这就是 §5.5
"共轭"一词的数学来源。

#### 等价写法（实现采用的、更省的"偏差式"）

把上式里的固定部分抽出来，可写成"**偏差**"形式：

```
偏差(j) ≜ Rb(j)⁻¹ ⊗ W(j)                     ← "该骨相对自己 rest 转了多少（世界系看）"
W_T(j)  = Rb_T(j) ⊗ 偏差_源(s(j))             …(★)
P_T(j)  = B_T(j)⁻¹ ⊗ W_T(parent)⁻¹ ⊗ W_T(j)   ← 反解局部量
```

与共轭式**语义等同**（见 `skeleton_retarget.h` 文件头的推导），但少一次乘法、少一层误差。
实现走 (★)，**同时把 Q 作为可检视的输出暴露出来**（诊断 / 人工校准用）。

#### 三个关键性质

1. **骨长（`rest_offset`）完全不参与** —— 公式里没有它。目标用自己的骨长 ⇒
   "两资产骨长不同"无需任何处理（印证 §6.2 表格"骨长走 SkeletonType 自身长度"）。
2. **两侧 bind 朝向不同（Z 轴位置不同）不是障碍，正是 Q 要吸收的东西**。
3. **根的整体旋转天然长在 Q 里**：`Q(0) = Rb_T(0) ⊗ Rb_S(0)⁻¹` 就是两侧**整体朝向差**
   （如源朝 +Z、目标朝 +X ⇒ Q(0) 是绕 up 的 90°）。**不是特例、不需单独处理** ——
   它就是同一公式在 j=0 处的取值。
   ⚠️ 连带后果：输出 pose **含角色朝向**。消费者若在放置层还叠 up/front，会**转向叠两次**，
   须二选一（见 `skeleton_retarget.h` "边界"一段）。

#### 如何验证（判据）

```
不变量 1：源 pose 恒等 ⟹ 目标 pose 恒等（目标保持自己的 T-pose）
不变量 2：命中骨 deviation_T(j) == deviation_S(s(j))（世界系偏差逐骨精确一致）
不变量 3：未命中的目标骨保持自身 rest
不变量 4：同布局骨架（两侧 bind 逐骨相同，只差骨长）⟹ Q ≈ 恒等，且输出 ≈ 输入
```

> **不变量 1 值得强调**（易被误解成 bug）：它**不是**"目标被摆成源的朝向"。
> 源不动 ⇒ 偏差恒等 ⇒ 目标也保持自己的 rest。这正是"目标是自己的形状、做源的同一段动作"这一设计意图。

#### 与"辅助姿态（auxiliary pose）"的关系

业界（Unity Avatar / retargeting-threejs）用"两侧各跟一个公共辅助姿态对齐一次"。
JPOV 的 Q 是**同一思想的直接形式**：不引入"辅助姿态"这个中间物，
直接把两侧的 `Rb` 相比 —— 更少概念、无中间数据的存放问题。

§2 的 Mixamo 布局约定仍是**规范化目标**（新资产要长成那样），但它不再是 retarget 的数学必需项。

---

### 1.2b 历史：旧的"辅助姿态"表述（保留备查）
...
| **rest 旋转** | **≈ 恒等** | FBX 实测 |
| **骨架空间**（skeleton space） | 根关节位于 `(0,0,0)`，直接 +Y 生长 | 本设计约定（见 §3）；即骨架自身坐标系原点 |
...
**一个带骨（skinned）的 mesh**，其关节坐标 = T-pose 时各关节的**骨架空间坐标**。
...
- **关节坐标 == T-pose 的骨架空间坐标** → 与 §1.2 的辅助姿态天然一致
...
Danis 原以为「glb 里的人物相对骨架空间有 scale、整体旋转、平移」。
