...
- **人群动画走「骨骼动画纹理」（骨骼烘焙蒙皮），作为 JPOV 官方渲染原语**：把角色的骨骼矩阵（非顶点）按
  ...
  **两个开销从 CPU 消失**，CPU 每帧只每人传一个小 instance(相位)。显存只随**骨数×帧**涨、**不随顶点数涨**。
  ...
  > **2026-09-18 修订**：纹理里每骨存的**不是 4×4 矩阵，而是对偶四元数（DQS，每骨 2 texel）**，
  > VS 的混合从「Σ w·M」换成 DLB 刚体混合（保体积、无糖纸）。对本文的成本结论只有好处：
  > 纹理内存减半（2048² RGBA32F：64MB→32MB）、每顶点取数 16→8 次。
  > 详见 `docs/jpov_dqs_skinning_design.md`。
