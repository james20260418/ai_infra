...
echo ""
echo "🧪 用法："
echo "   $OUTPUT_DIR/jpov_fbx_viewer /absolute/path/to/anim.fbx"
...
echo "   ② [固定 rest 位姿(identity)] 复选框 —— 勾上切到 rest 模式：不看动画，固定"
echo "      pose = identity 渲染骨架 rest（T-pose）形态；此时时间同样不推进，"
echo "      取消勾选后从停住的时刻继续播放。"
echo "   ③ 状态行：帧号 / 总帧数 / 源帧频 / 动画时刻 / 播放 or 已暂停。"
echo ""
echo "   播放速度 = FBX 自身的 fps（60fps 渲染下每帧推进 1/60 秒，帧间逐骨 Slerp 插值），"
echo "   循环播放（末帧回绕到首帧，接缝处同样插值）。"
echo ""
echo "📷  headless 出图（不弹窗，AI 自查/留证用）："
echo "   $OUTPUT_DIR/jpov_fbx_viewer <fbx> --shot /tmp/a.png [--time 秒 | --frame 帧号] [--rest]"
echo "   → 指定时刻（或帧号）的纯 3D 截图；--rest 出 identity 位姿图。"
